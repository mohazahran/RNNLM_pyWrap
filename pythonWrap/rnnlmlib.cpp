...
void CRnnLM::readWord(char *word, FILE *fin)
{
    ...
    while (!feof(fin))
    {
        ...
        if (ch==13) continue; //carrige return
...
double CRnnLM::testNetOne(std::string line)
{
    ...
    if (bptt>0)
        for (a=0; a<bptt+bptt_block; a++) bptt_history[a]=0;
    ...
    std::string myWord; // Have a buffer string
    std::stringstream ss(line); // Insert the string into a stream
    
    while (ss >> myWord)
    {
        char *cstr_currentWord = (char*) myWord.c_str();
        //cout<<cstr_currentWord<<" ";
        word=searchVocab(cstr_currentWord);		//read next word
        //cout<<word<<endl;
        computeNet(last_word, word);		//compute probability distribution
        //if (feof(fi)) break;		//end of file: report LOGP, PPL
